...
class MainWindow(QMainWindow):
    ...
    def encodeMsg(self):
        ...
        for i in range(4):
            byte_array.push_front(bytes(((self.byte_list_size >> i * 8) & 0xff,)))
        #
        for i in range(len(self.marker) - 1, -1, -1):
            byte_array.push_front(bytes((self.marker[i],)))

        str_text = self.ui.plainTextEdit.toPlainText()
        #byte_list_str_text = str_text.encode()
        #byte_array.push_back(byte_list_str_text)
        index = 0
        #stop = 0
        stop = (index +  byte_array.size())*8
        for i in range(index * 8,stop):
            x = (i // 3) % self.image.width()
            y = (i // 3) // self.image.width()
            shift = (int.from_bytes(byte_array[i // 8], "big" )>> (7 - i % 8)) & 1
            rez = i % 3
            if (rez == 0):
    ...
    def decodeMsg(self):
        ...
        for i in range (index * 8,stop,1):
            temp = (temp << 1) | get_bit(self.image, i)
            if (i % 8 == 7):
        ...
        for i in range(index2 * 8,stop2):
            temp2 = (temp2 << 1) | get_bit(self.image, i)

            if (i % 8 == 7):
                byte_array2.push_back(bytes((temp2,)))
                temp2 = 0



        self.ui.plainTextEdit.setPlainText(bytes(byte_array2).decode("utf-8"))
