...
public:
    ...
    {
        return abs(b) ? gcd (abs(b), abs(a % b)) : abs(a);
    }

    bool operator==(const Rational& other) const
    {
        if (isNaN() || other.isNaN()) return false;
        else if (get_num() == 0 && other.get_num() == 0) return true;
        else if (sign != other.sign) return false;
        else if (get_denom() == 0 && other.get_denom() == 0) return true;
//        if (get_num()*other.get_denom() == get_denom()*other.get_num()) return true;
//        return get_num() == other.get_num() && get_denom() == other.get_denom();
    ...
    Rational operator+(const Rational& other) const
    {
        ...
        {
            if (sign == other.sign) return Rational(sign, 0);
            return Rational(0,0);
        }
        else if (get_denom() == 0) return *this;
        else if (other.get_denom() == 0) return other;
        ...
        else
        {
            ...
            if (sign == 1 and other.sign == 1) temp.sign = 1;
            else if (sign == -1 and other.sign == -1) temp.sign = -1;
        }
    ...
    Rational operator*(const Rational& other) const {
        Rational temp;
        temp.sign = sign*other.sign;
        temp.numerator = get_num()*other.get_num();
        temp.denominator = get_denom()*other.get_denom();
        return temp;
    }
    ...
    Rational operator/(const Rational& other) const {
        Rational temp;
        temp.sign = sign*other.sign;
        temp.numerator = get_num()*other.get_denom();
        temp.denominator = get_denom()*other.get_num();
        return temp;
    }
    ...
    operator bool() const {
        if (get_num() == 0 && get_denom() == 0) return true;
        else if (get_num() == 0) return false;
        else return true;
    }


    bool isNaN() const { return get_num() == 0 && get_denom() == 0;}
